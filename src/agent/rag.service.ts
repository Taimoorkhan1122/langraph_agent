...
import { buildRagReferences, formatInlineReferences } from './rag-reference';
...
export class RagService {
  ...
  async query(
    ...
    const references = buildRagReferences(sources);

    const answer =
      sources.length > 0
        ? `${sources.map((s) => s.answer).join('\n\n')}\n\nBased on ${formatInlineReferences(references)}.`
